...
class Replay_Buffer:
    ...
    def push (self, state : State, action, reward, next_state: State, done):
        actions = torch.from_numpy(np.array(action)).reshape(-1,4)
        self.buffer.append((state.toTensor(), actions, torch.tensor(reward), next_state.toTensor(), torch.tensor(done)))
